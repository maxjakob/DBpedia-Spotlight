        ...
        override def foreach[U](f : DBpediaResourceOccurrence => U) {
            ...
            for (line <- linesIterator) {
                ...
                if (elements.length == 5) {
                    val id = elements(0)
                    val res = Factory.DBpediaResource.from(elements(1), 1) // support is at least one if this resource has been seen once here
        ...
        override def foreach[U](f : WikiPageContext => U) : Unit =
        {
            ...
            for (line <- Source.fromInputStream(input, "UTF-8").getLines) {
                try {
                    ...
                    {
                        val res = Factory.DBpediaResource.from(elements(0), 1)
